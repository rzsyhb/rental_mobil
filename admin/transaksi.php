    ...
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-soft align-middle mb-0">
                ...
                <tbody>
                <?php if ($query && mysqli_num_rows($query) > 0) { $no=1; while($d = mysqli_fetch_assoc($query)){ $statusEfektif = status_transaksi_efektif($d); $d['status'] = $statusEfektif; $ot = hitung_overtime_data($d); 
                $totalTagihan = total_tagihan_transaksi($d); $ktpUrl = secure_file_url('customer_ktp', $d['id_customer']); $simUrl = secure_file_url('sim_a', $d['id_transaksi']); ?>
