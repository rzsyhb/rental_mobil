...
{
    $status = strtolower(trim((string) ($mobil['status'] ?? 'tersedia')));
...
function sinkronisasi_status_mobil($conn)
{
    ...
    /*
     * Hanya ubah ke disewa jika memang ada rental aktif.
     * Jangan paksa semua mobil menjadi tersedia.
     */
...
function transaksi_status_info($row)
{
    ...
    if ($status === 'dibatalkan') {
        return ['label' => 'Dibatalkan', 'class' => 'danger'];
    }
    if ($status === 'selesai') {
...
function app_parse_datetime_timestamp($value)
{
...
function transaksi_sudah_dibatalkan_atau_selesai($row)
{
    $status = strtolower(trim((string) ($row['status'] ?? '')));
    $statusFinal = [
        'dibatalkan', 'batal', 'cancel', 'cancelled', 'batal admin', 'dibatalkan admin',
        'selesai', 'complete', 'completed', 'selesai admin'
    ];

    if (in_array($status, $statusFinal, true)) {
        return true;
    }

    if (!empty($row['selesai_admin_at']) && trim((string) $row['selesai_admin_at']) !== '0000-00-00 00:00:00') {
        return true;
    }

    $catatan = strtolower((string) ($row['catatan_admin'] ?? ''));
    if (strpos($catatan, 'batal') !== false || strpos($catatan, 'dibatalkan') !== false) {
        return true;
    }
...
function ambil_jadwal_mobil_dipakai($conn, $idMobil, $excludeId = 0)
{
    $idMobil = (int) $idMobil;
    ...
    foreach (['id_mobil', 'tanggal_sewa', 'tanggal_kembali'] as $kolom) {
        if (!table_has_column($conn, 'transaksi', $kolom)) {
            return $jadwal;
        }
    }

    $select = [
        table_has_column($conn, 'transaksi', 'id_transaksi') ? 'id_transaksi' : '0 AS id_transaksi',
        'id_mobil',
        'tanggal_sewa',
        'tanggal_kembali',
        table_has_column($conn, 'transaksi', 'status') ? 'status' : "'menunggu pembayaran' AS status",
        table_has_column($conn, 'transaksi', 'catatan_admin') ? 'catatan_admin' : "'' AS catatan_admin",
        table_has_column($conn, 'transaksi', 'selesai_admin_at') ? 'selesai_admin_at' : 'NULL AS selesai_admin_at',
        table_has_column($conn, 'transaksi', 'bukti_pembayaran') ? 'bukti_pembayaran' : "'' AS bukti_pembayaran",
        table_has_column($conn, 'transaksi', 'sim_a') ? 'sim_a' : "'' AS sim_a",
        table_has_column($conn, 'transaksi', 'overtime_jam') ? 'overtime_jam' : '0 AS overtime_jam',
        table_has_column($conn, 'transaksi', 'overtime_biaya') ? 'overtime_biaya' : '0 AS overtime_biaya',
    ];

    $where = [
        "id_mobil='$idMobil'",
        "tanggal_sewa IS NOT NULL",
        "tanggal_kembali IS NOT NULL",
    ];

    if ($excludeId > 0 && table_has_column($conn, 'transaksi', 'id_transaksi')) {
        $where[] = "id_transaksi != '$excludeId'";
    }

    $sql = "SELECT " . implode(', ', $select) . " FROM transaksi WHERE " . implode(' AND ', $where) . " ORDER BY tanggal_sewa ASC";
    ...
    $now = time();
    while ($row = mysqli_fetch_assoc($result)) {
        if (transaksi_sudah_dibatalkan_atau_selesai($row)) {
            continue;
        }

        $mulaiTs = app_parse_datetime_timestamp($row['tanggal_sewa'] ?? '');
        $selesaiTs = app_parse_datetime_timestamp($row['tanggal_kembali'] ?? '');
        if ($mulaiTs === false || $selesaiTs === false || $selesaiTs <= $mulaiTs) {
            continue;
        }

        $statusEfektif = strtolower(trim((string) status_transaksi_efektif($row)));

        /*
         * Transaksi menunggu pembayaran/konfirmasi yang waktunya sudah lewat tidak perlu
         * mengunci jadwal. Transaksi terverifikasi/disewa tetap mengunci sampai admin
         * menekan tombol selesai, karena masih ada proses pengembalian/overtime.
         */
        if ($selesaiTs <= $now && !in_array($statusEfektif, ['terverifikasi', 'disewa'], true)) {
            continue;
        }

        $infoStatus = transaksi_status_info($row);
        $jadwal[] = [
            'id_transaksi' => (int) ($row['id_transaksi'] ?? 0),
            'start' => date('Y-m-d\\TH:i', $mulaiTs),
            'end' => date('Y-m-d\\TH:i', $selesaiTs),
            'start_sql' => date('Y-m-d H:i:s', $mulaiTs),
            'end_sql' => date('Y-m-d H:i:s', $selesaiTs),
            'status' => (string) ($row['status'] ?? ''),
            'label' => $infoStatus['label'],
            'class' => $infoStatus['class'],
            'is_future' => $mulaiTs > $now,
            'is_running' => $mulaiTs <= $now && $selesaiTs > $now,
            'is_overdue_unfinished' => $selesaiTs <= $now && in_array($statusEfektif, ['terverifikasi', 'disewa'], true),
        ];
    }
...
function ada_bentrok_jadwal($conn, $idMobil, $mulai, $selesai, $excludeId = 0)
{
    ...
    if ($idMobil <= 0 || empty($mulai) || empty($selesai)) {
        return true;
    }

    $mulaiTs = app_parse_datetime_timestamp($mulai);
    $selesaiTs = app_parse_datetime_timestamp($selesai);
    if ($mulaiTs === false || $selesaiTs === false || $selesaiTs <= $mulaiTs) {
        return true;
    }

    $jadwalDipakai = ambil_jadwal_mobil_dipakai($conn, $idMobil, $excludeId);
    foreach ($jadwalDipakai as $jadwal) {
        $jadwalMulai = app_parse_datetime_timestamp($jadwal['start_sql'] ?? $jadwal['start']);
        $jadwalSelesai = app_parse_datetime_timestamp($jadwal['end_sql'] ?? $jadwal['end']);
        if ($jadwalMulai === false || $jadwalSelesai === false) {
            continue;
        }

        // Rumus bentrok: mulai baru < selesai lama DAN selesai baru > mulai lama.
        if ($mulaiTs < $jadwalSelesai && $selesaiTs > $jadwalMulai) {
            return true;
        }
    }
...
function app_normalize_allowed_image_extensions($allowedExtensions = null)
{
...
function app_uploaded_image_is_allowed($tmpPath, $extension, $allowedExtensions)
{
    $extension = strtolower((string) $extension);
    $allowedExtensions = app_normalize_allowed_image_extensions($allowedExtensions);

    if (!in_array($extension, $allowedExtensions, true) || !is_uploaded_file($tmpPath)) {
        return false;
    }
    ...
    foreach ($allowedExtensions as $ext) {
        if (isset($mimeByExtension[$ext])) {
            $allowedMimes[] = $mimeByExtension[$ext];
        }
    }
...
function upload_gambar($fieldName, $targetDir, $allowedExtensions = null)
{
    ...
    $allowed = app_normalize_allowed_image_extensions($allowedExtensions);
    if (!app_uploaded_image_is_allowed($_FILES[$fieldName]['tmp_name'], $ext, $allowed)) {
        return '';
    }
...
function upload_gambar_terenkripsi($fieldName, $category, $allowedExtensions = null)
{
