...
/*
 * Dashboard customer membaca data utama dari tabel mobil terlebih dahulu.
 * Status sewa/jadwal tetap dicek dari transaksi, tetapi tidak membuat armada kosong
 * jika tabel transaksi sedang direvisi atau ada kolom tambahan yang belum terbentuk.
 */
$mobilQuery = mysqli_query($conn, "SELECT * FROM mobil ORDER BY id_mobil DESC");
...
if ($mobilQuery) {
    while ($row = mysqli_fetch_assoc($mobilQuery)) {
        $row['sedang_disewa'] = 0;
        $row['jadwal_terdekat'] = null;

        if ($transaksiSiap) {
            $idMobil = (int) $row['id_mobil'];
            ...
            if ($cekAktif) {
                ...
                $row['sedang_disewa'] = ((int) ($rowAktif['total'] ?? 0) > 0) ? 1 : 0;
            }
            ...
            if ($cekJadwal) {
                ...
                $row['jadwal_terdekat'] = $rowJadwal['jadwal_terdekat'] ?? null;
            }
        }

        if ((int) $row['sedang_disewa'] === 1 || ($row['status'] ?? '') === 'disewa') {
            $aktif[] = $row;
        } elseif (($row['status'] ?? '') !== 'sedang diservice') {
            $tersedia[] = $row;
        }
    }
}
